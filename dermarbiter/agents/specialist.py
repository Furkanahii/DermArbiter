...
class SpecialistAgent(BaseAgent):
    ...
    def generate_brief(
        ...
        prompt = (
            ...
            "═══ CLASS PRIORS (HAM10000 dataset distribution) ═══\n"
            "These are the actual population frequencies in the benchmark.\n"
            "Apply them as Bayesian priors when evidence is non-specific:\n"
            "  nv (melanocytic_nevus):     67%  ← BY FAR the most common\n"
            "  mel (melanoma):             11%\n"
            ...
            "  vasc (vascular):             1%\n\n"
            "DECISION RULES:\n"
            "  • Default to 'melanocytic_nevus' (nv) when features are ambiguous "
            "or non-specific. Most pigmented lesions in this population are benign nevi.\n"
            "  • Only call 'melanoma' (mel) when you see UNAMBIGUOUS malignant "
            "features (frank asymmetry + multiple colors + irregular network + "
            "regression structures). Defensive over-calling of mel destroys "
            "precision and is clinically harmful (unnecessary biopsies).\n"
            "  • Only call 'benign_keratosis' (bkl) when you see SPECIFIC keratotic "
            "features (comedo-like openings, milia-like cysts, sharply demarcated "
            "border). Don't default to bkl just because the lesion looks 'atypical'.\n"
            "  • Use HAM10000 7-class codes EXACTLY in your top3_differential: "
            "['nv', 'mel', 'bkl', 'bcc', 'akiec', 'df', 'vasc']. NOT free-text "
            "like 'atypical melanocytic nevus' (that maps to 'nv').\n\n"
            f"{evidence_context}\n\n"
            "Respond with ONLY a JSON object in this exact schema:\n"
            "```json\n"
            "{\n"
            '  "top3_differential": ["<HAM10000 code>", "<HAM10000 code>", "<HAM10000 code>"],\n'
            '  "confidence": 0.XX,\n'
            ...
            "- Top3 differential MUST use the 7 HAM10000 codes (nv/mel/bkl/bcc/akiec/df/vasc), "
